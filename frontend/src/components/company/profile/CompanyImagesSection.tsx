...
import type { CompanyImage } from "@/hooks/company/useProfile";
import {
  useDeleteCompanyImage,
  useUploadCompanyImage,
} from "@/hooks/company/useCompanyImages";

// バックエンドのUpdateImageRequestと同じ制約(jpeg/png/webp、5MB以内)。
// アップロード前に弾ける明らかな不正入力はここでチェックし、それ以外は
// サーバー側のバリデーションエラーをそのまま表示する(JobPostingImagesSectionと同じ方針)
const ALLOWED_TYPES = ["image/jpeg", "image/png", "image/webp"];
const MAX_SIZE_BYTES = 5120 * 1024;
const MAX_IMAGES = 5;

export function CompanyImagesSection({ images }: { images: CompanyImage[] }) {
  const uploadMutation = useUploadCompanyImage();

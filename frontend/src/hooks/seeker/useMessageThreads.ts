...
import type { LikeStatus, LikeType } from "@/hooks/seeker/useLikes";
...
// バックエンドのSeeker\MessageThreadController@indexが返す形と対応
...
export const messageThreadsQueryKey = ["seeker", "messageThreads"] as const;

function threadMessagesQueryKey(likeId: number) {
  return ["seeker", "messageThreads", likeId, "messages"] as const;
}

export function useMessageThreads() {
  return useQuery({
    queryKey: messageThreadsQueryKey,
    queryFn: () => apiFetch<MessageThread[]>("/api/message-threads"),
  });
}
...
export function useThreadMessages(likeId: number) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<Message[]>(`/api/message-threads/${likeId}/messages`),
  });
}
...
export function useHideThread() {
  ...
  return useMutation({
    mutationFn: (likeId: number) =>
      apiFetch<void>(`/api/message-threads/${likeId}/hide`, {
        method: "PATCH",
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: messageThreadsQueryKey });
